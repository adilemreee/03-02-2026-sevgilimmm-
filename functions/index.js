...
// =============================================
// LOCATION & MEETING FUNCTIONS
// =============================================

/**
 * Konum güncellendiğinde yakınlık kontrolü yap
 * Eğer partner yakınsa buluşma bildirimi gönder
 */
exports.onLocationUpdate = functions.firestore
    .document("userLocations/{userId}")
    .onWrite(async (change, context) => {
      ...
      // Konum paylaşımı aktif değilse çık
      if (!newData.isSharing) {
        return null;
      }

      try {
        // Kullanıcının relationship'ini bul
        ...
        // Partner'ın konumunu al
        const partnerLocationDoc = await firestore
            .collection("userLocations")
            .doc(partnerId)
            .get();

        if (!partnerLocationDoc.exists) {
          return null;
        }

        const partnerLocation = partnerLocationDoc.data();

        // Partner konum paylaşmıyorsa çık
        if (!partnerLocation.isSharing) {
          return null;
        }
        ...
        // Yakınlık eşiği (metre) - varsayılan 100m
        const proximityThreshold = 100;

        // Yakınlar mı?
        if (distance <= proximityThreshold) {
          // Aktif buluşma var mı kontrol et
          const activeMeetingSnapshot = await firestore
          ...
          if (activeMeetingSnapshot.empty) {
            // Yeni buluşma başlat
            const meetingData = {
              ...
              latitude: (newData.latitude + partnerLocation.latitude) / 2,
              longitude: (newData.longitude + partnerLocation.longitude) / 2,
              isActive: true,
            ...
            await meetingRef.update({notificationSent: true});
          } else {
            // Mevcut buluşmadaki mesafeyi güncelle
            const meetingDoc = activeMeetingSnapshot.docs[0];
            const meetingData = meetingDoc.data();

            const updates = {};
            if (distance < meetingData.minDistance) {
              updates.minDistance = distance;
            }
            if (distance > meetingData.maxDistance) {
              updates.maxDistance = distance;
            }
          ...
          // Uzaktalar - aktif buluşma varsa bitir
          const activeMeetingSnapshot = await firestore
              .collection("meetingEvents")
              .where("relationshipId", "==", relationshipId)
              .where("isActive", "==", true)
          ...
          if (!activeMeetingSnapshot.empty) {
            ...
            // Relationship istatistiklerini güncelle
            await relationshipDoc.ref.update({
              totalMeetingCount: admin.firestore.FieldValue.increment(1),
              totalMeetingDuration: admin.firestore.FieldValue.increment(duration),
              lastMeetingDate: meetingData.startTime,
              lastMeetingLatitude: meetingData.latitude,
              lastMeetingLongitude: meetingData.longitude,
            });
        ...
        logger.error("Error in onLocationUpdate", {userId, error});
        return null;
      }
    });

/**
 * Buluşma bildirimi gönder
 */
async function sendMeetingNotification(userIds, relationship, meetingId) {
  try {
    const userMap = await loadUsers(userIds);
    ...
    for (const userId of userIds) {
      ...
      // Partner adını bul
      const partnerName = relationship.user1Id === userId ?
          relationship.user2Name : relationship.user1Name;

      const notification = {
        title: "Buluştunuz! 💕",
        body: `${partnerName} ile buluştunuz!`,
      };
      ...
      const tokens = normaliseTokens(user.fcmTokens);
      if (tokens.length === 0) {
        continue;
      }

      const baseMessage = applyDefaultApns({
        notification,
      ...
      for (const token of tokens) {
        try {
          ...
          logger.warn("Failed to send meeting notification", {
            userId,
    ...
    logger.error("Error sending meeting notification", {userIds, error});
  }
}
...
/**
 * Pending notifications işle
 * (Uygulama tarafından oluşturulan bildirimleri gönder)
 */
      ...
      try {
        ...
        for (const token of tokens) {
          try {
            ...
            logger.warn("Failed to send pending notification", {
              notificationId,
              error: sendError.message,
            });
          }
...
/**
 * Eski konumları temizle (günlük çalışır)
 * 24 saatten eski ve paylaşımı kapalı konumları siler
 */
exports.cleanupOldLocations = functions.pubsub
    ...
    .onRun(async (context) => {
      ...
      try {
        ...
        if (count > 0) {
          await batch.commit();
          logger.info("Cleaned up old locations", {count});
        }
...
/**
 * Terk edilmiş buluşmaları temizle (günlük çalışır)
 * 1 saatten uzun aktif buluşmaları otomatik bitir
 */
exports.cleanupStaleMeetings = functions.pubsub
    .schedule("every 1 hours")
    .onRun(async (context) => {
      const cutoffDate = new Date();
      ...
      try {
        ...
        for (const doc of snapshot.docs) {
          ...
          // Relationship istatistiklerini güncelle
          if (meetingData.relationshipId) {
            const relRef = firestore
                .collection("relationships")
                .doc(meetingData.relationshipId);
          ...
          logger.info("Auto-ended stale meeting", {
            meetingId: doc.id,
            duration,
          });
        }
        ...
        logger.error("Error cleaning up stale meetings", {error});
        return null;
      }
    });
